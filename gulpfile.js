...
const siteDestDirectory = 'site';

gulp.task('watch', () => {
    gulp.watch('themes/hugo-bootstrap/src/scss/**/*.scss', ['styles']);
});

gulp.task('styles', ['clean:styles'], () =>
  gulp.src('themes/hugo-bootstrap/src/scss/**/*.scss')
...
gulp.task('minify:markup', () =>
  gulp.src(siteDestDirectory + '/**/*.html')
    .pipe(htmlmin({ collapseWhitespace: true }))
    .pipe(inline({
      rootpath: siteDestDirectory + '/',
      handlers: (source, context, next) => {
        if (source.type === 'css' && source.fileContent && !source.content) {
          uncss(context.html, { htmlroot: siteDestDirectory }, (error, css) => {
    ...
    .pipe(gulp.dest(siteDestDirectory))
);
